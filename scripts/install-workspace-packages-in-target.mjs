...
if (!fs.existsSync(nodeModulesPath)) {
  console.error(`Target directory "${targetDir}" does not have a node_modules directory`)
  process.exit(1)
}

// validate that the node_modules directory is a directory
if (!fs.statSync(nodeModulesPath).isDirectory()) {
  console.error(`Target directory "${targetDir}" node_modules directory is not a directory`)
  process.exit(1)
}
...
const packages = fs
  ...
  .filter((file) => fs.statSync(path.join(nodeModulesPath, file)).isDirectory())

console.log(`Found ${packages.length} packages to merge: ${packages.join(", ")}`)
// #endregion
