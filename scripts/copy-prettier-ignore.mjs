...
console.log(`found ${workspacePaths.length} workspaces: ${workspacePaths.join(", ")}`)

// copy the .prettierignore file to each workspace
for (const workspacePath of workspacePaths) {
  const workspacePrettierIgnorePath = path.join(workspacePath, ".prettierignore")
  ...
  if (fs.existsSync(workspacePrettierIgnorePath)) {
    ...
    console.log(`removed existing ".prettierignore" at ${workspacePrettierIgnorePath}`)
  }
