...
function buildViewMenu(window: Electron.BrowserWindow, isDevelopment: boolean) {
    const viewMenu = {
        label: isDarwin ? 'View' : '&View', submenu: [],
    };
    ...
    if (isDevelopment) {
        viewMenu.submenu.push({
            label: isDarwin ? 'Reload' : '&Reload', accelerator: isDarwin ? 'Command+R' : 'Ctrl+R', click: () => {
...
function buildWindowMenu(window: Electron.BrowserWindow) {
    ...
    return {
        label: 'Window', submenu: [
            {
                type: 'checkbox', label: 'Always On Top', click: () => {
